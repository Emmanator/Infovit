...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        greedy.append([greedy_solution.__str__(), greedy_score])
    sorted_greedy = sorted(greedy, key=lambda x: max(x[1:]), reverse=True)
    # Simulated annealing
    annealing = []
    for i in range(100):
        sim_anneal_solution = sol.simulated_annealing_solve()
        annealing_score = (sol.score(sim_anneal_solution))
        annealing.append([sim_anneal_solution.__str__(), annealing_score])
    sorted_annealing = sorted(annealing, key=lambda x: max(x[1:]), reverse=True)

    # print("Greedy search: " + str(greedy_solution))
    print(f'Greedy score: {str(sorted_greedy[0][0])}, {str(sorted_greedy[0][1])}')

    # print("\nSimulated annealing search: " + str(sim_anneal_solution))
    print(f'Simulated annealing score: {str(sorted_annealing[0][0])}, {str(sorted_annealing[0][1])}')
